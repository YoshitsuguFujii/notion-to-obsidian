...
1. `sync --full --dry-run --strict`を実行し、UPDATE範囲を確認します。v1から移行する場合は、`asset_signed_url_replaced`のWARNINGも確認します。署名付きURLの検出WARNINGによる非0終了が想定されるのはv1からの移行時です。安全エラーやpartial censusによる非0終了とは区別し、後者がある場合は実同期へ進まず原因を解消してください。署名付きURLの終端を安全に判断できない旨の安全エラーが出た場合も、本文を確認してから再実行してください。
