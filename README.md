...
`plan`と`sync`は対象ページ数に比例して時間がかかります。censusがページごとに子ブロックを再帰的に取得し、`notion.request_rate_per_second`（既定2.5）に律速されるためです。実測では31ページで20分以上、1ページあたり約40秒でした。census実行中は発見済みページ数と実行済みリクエスト数を5秒間隔でstderrへ`logging.level: info`のログとして出力するため、動作中かどうかはログの進捗で判断できます（ログファイルは作成されません）。`--verbose`を付けるとリクエスト単位でさらに詳細なログが出ます。停止したように見えても、ログが進んでいる間は中断せず完了かエラーまで待ってください。
...
- transform version 5の導入前に同期を済ませており、半角括弧・強調記号・英数字が密着した署名付きURLの直後の本文が欠落していた可能性がある利用者

transform version 5では、非ASCIIを含むパスの安定参照がパーセントエンコード形式（`…/画像.png`から`…/%E7%94%BB%E5%83%8F.png`）になります。指す先は同じですが本文の文字列が変わるため、`--full`のUPDATE差分としてこれが現れることがあります。安全エラーとは異なるので区別してください。

新規導入時や通常運用で`--full`を常用する必要はありません。transform version・APIバージョン・設定内容は、そのページを実際に生成・書き込みした時点のprovenanceとしてページごとにstate DBへ記録され、次回以降の通常`sync`は現在の値と比較して自動的に再生成対象を検出します。したがって、上記の一覧にある過去のtransform version導入より**後**に本ツールを使い始めた利用者、または既にこの自動検出を経験した利用者は、この手順を実行する必要がありません。

このprovenance追跡機構の導入（自動検出そのものが直った版）に限り、導入前に同期していたページは一度だけ通常の`sync`でUPDATEされ、以降は現在値に収束します。この一度限りのUPDATEでは、内容が変わらないページも含めてMarkdownの更新日時（mtime）が変わります。バックアップや外部ツールとの同期をmtimeに依存している場合は、この一度限りの変化を見込んでください。

1. `sync --full --dry-run --strict`を実行し、UPDATE範囲を確認します。v1から移行する場合は、`asset_signed_url_replaced`のWARNINGも確認します。署名付きURLの検出WARNINGによる非0終了が想定されるのはv1からの移行時です。安全エラーやpartial censusによる非0終了とは区別し、後者がある場合は実同期へ進まず原因を解消してください。署名付きURLの範囲を構文から確定できない安全エラーには該当ページIDが表示されます。範囲が確定するのは、Markdownのリンク・画像のURL部分、`<URL>`形式、HTMLの引用符付き属性値の3つだけです。裸のURLは停止します。**URLだけの行のように値全体がURLの場合も停止する**ため、行へ分離しても解消しません。本文中のURLは、Notion上で`[表示文字列](URL)`・`![alt](URL)`・`<URL>`のいずれかの形にしてから、`sync --full --dry-run --strict`で解消を確認して実同期してください。コードブロックやインラインコード内の裸のURLも同じく停止します。これらはコード例の見た目を変えずに直せないため、署名付きURLをコード例に含めないよう書き換えるか、対象ページを同期対象から外してください。

   ページタイトル、URL型プロパティ、Data Sourceの表示文字列に署名付きURLが入っている場合は、これらがMarkdown記法を保持できないため、現時点では**Notion側で当該の値から署名付きURLを取り除く以外に解消手段がありません**。本ツールはNotionへ書き込まないので、この操作は利用者がNotion上で行う必要があります。取り除けない場合は、対象ページを同期対象から外してください。型からURLだと分かる箇所を個別に扱えるようにする恒久対応は今後行います。
...
コードフェンス内とinline code内ではMarkdownのリンク構文が解釈されないため、そこに書かれたNotion署名付きURLは置換せず安全エラーで停止します。署名原文をそのまま保存することはありません。置換の発生はURLを含まないWARNINGと件数で確認できます。
