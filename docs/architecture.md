...
- アセット（`_assets/<pageId>/<blockId>--<name>.<ext>`）は一時ファイルへダウンロードし、`_assets` を予約領域として content hash で所有を判定してから確定する。ダウンロード内容の sha256 を計算し、既存 target は「不在＝排他確保」「disk が今回の内容と一致＝変更せず取り込み（adopt）」「disk が state DB の記録した旧 hash と一致＝新内容へ置換（正当な更新）」「いずれでもない＝停止」「非通常・読取不能＝停止」と分岐する。ダウンロード自体の失敗（ネットワーク・サイズ超過・許可外・不正 URL）は従来どおり warning で同期を継続しリモート URL を残すが、保存先の所有 conflict・非通常 target・読取不能は同期を停止する。dry-run は未取得アセットの内容判定を Apply へ延期し、判定できない保存先を `ASSET_DEFERRED` として表示する（dry-run 成功は Apply 成功を完全保証しない）。resource と同ページのアセットの state DB 更新は同一 transaction で行い、片方だけの反映を避ける。既存ファイルの内容変更検知（ローカルに存在する間は remote の変更を検知しない既知の制約）と、UNCHANGED ページに取り残されたアセットの取り込みは今後の対応。
